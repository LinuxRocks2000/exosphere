...
pub fn linear_maneuvre(
    delta: Vec2,
    current_velocity: Vec2,
    cap_speed: f32,
    nominal_acceleration: f32,
) -> f32 {
...
pub fn loopify(one: f32, two: f32) -> f32 {
    // return the circle displacement between two angles
    ...
    let rdist = two - one;
    if rdist > PI {
        // if we would have to travel more than 180 degrees, there is guaranteed to be a better solution by loopifying
...
pub fn space_gyro(
    ...
    goal_position: f32,
    current_rotational_velocity: f32,
    cap_rotational_velocity: f32,
    nominal_radial_acceleration: f32,
) -> f32 {
    ...
    } else if current_rotational_velocity.signum() == delta.signum() {
