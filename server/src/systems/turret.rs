...
pub fn turrets(mut turrets : Query<(&mut Turret, &GamePiece, &mut ExternalImpulse, &Transform, &Velocity)>, pieces : Query<(&GamePiece, &Transform, &Velocity)>) {
    for (mut turret, turret_piece, mut impulse, turret_pos, turret_velocity) in turrets.iter_mut() {
        for i in 0..turret.in_range.len() {
            if let Ok((piece, position, velocity)) = pieces.get(turret.in_range[i]) {
                ...
                if turret.targeting_algorithm.will_attack(piece.tp) {
                    let ang = turret.targeting_algorithm.get_target_angle((position.translation - turret_pos.translation).truncate(), velocity.linvel);
