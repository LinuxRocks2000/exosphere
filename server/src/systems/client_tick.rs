...
pub fn client_tick(mut commands : Commands, mut pieces : Query<(Entity, &GamePiece, Option<&mut Spaceshipoid>, Option<&Transform>, Option<&Territory>)>, mut guns : Query<&mut Gun>, mut ev_newclient : EventWriter<NewClientEvent>, place : EventWriter<PlaceEvent>, mut state : ResMut<GameState>, config : Res<GameConfig>, mut clients : ResMut<ClientMap>, mut receiver : ResMut<Receiver>, broadcast : ResMut<Sender>, mut client_killed : EventWriter<ClientKilledEvent>) {
                ...
                match message {
                    ...
                    Comms::MessageFrom(id, msg) => {
                        ...
                        if clients.contains_key(&id) {
                            match msg {
                                ClientMessage::Connect { nickname, password : _password } => { // TODO: IMPLEMENT PASSWORD
                                    let slot : u8 = if state.currently_attached_players < config.max_player_slots { 1 } else { 0 }; // todo: implement teams
                                    for k in clients.keys() {
                                        if *k != id {
                                            let message = ServerMessage::PlayerData { id : *k, nickname : clients[k].nickname.clone(), slot : clients[k].slot };
                                            clients[&id].send(message);
                                        }
                                    }
                                    clients.get_mut(&id).unwrap().send(ServerMessage::Metadata { id, board_width : config.width, board_height : config.height, slot });
                                    if let Err(_) = broadcast.send(ServerMessage::PlayerData { id, nickname : nickname.clone(), slot }) {
                                ...
                                ClientMessage::StrategyInsert { piece, index, node } => {
                                    if state.playing && state.strategy {
                                        if let Ok((_, piece, shipoid, _, _)) = pieces.get_mut(piece.into()) {
                                ...
                                ClientMessage::StrategySet { piece : piece_id, index, node } => {
                                    if state.playing && state.strategy {
                                        if let Ok((_, piece, shipoid, _, _)) = pieces.get_mut(piece_id.into()) {
                                            if let Some(mut shipoid) = shipoid {
                                                if piece.owner == id {
                                ...
                                ClientMessage::StrategyDelete { piece : piece_id, index } => {
                                    if state.playing && state.strategy {
                                        if let Ok((_, piece, shipoid, _, _)) = pieces.get_mut(piece_id.into()) {
                                            if let Some(mut shipoid) = shipoid {
                                                if piece.owner == id {
