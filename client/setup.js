...
function toMouseX(x) {
    return Math.min(Math.max(x - window.innerWidth/2 + window.exosphere.board.offX, 0), window.exosphere.board.width);
}

function toMouseY(y) {
    return Math.min(Math.max(y - window.innerHeight/2 + window.exosphere.board.offY, 0), window.exosphere.board.height);
}
...
function setup(state) {
    ...
    const mainloop = () => {
        ...
        state.set_mouse_pos(toMouseX(window.exosphere.board.rawMx), toMouseY(window.exosphere.board.rawMy));
        state.tick();
        exosphere.ctx.translate(-offX, -offY);
